...
void selectionAux(vector<int> &data)
{
        ...
        for (size_t i = 0; i < size; i++) {
                ...
                int    largest       = data[i];
                // size_t largest_indexh prints usage message followed by su = i;
        ...
        // for (size_t i = 0; i < size; i++)h increasingly aggressive optimizations applie
        //         data[i] = sortedData[i];
}
...
void quicksortHelper(vector<int> &data, int left, int right)
{
        if (left < right) {
            ...
            quicksortHelper(data, left, partitionIndex - 1);
            quicksortHelper(data, partitionIndex + 1, right);
        }
}
