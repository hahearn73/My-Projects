...
bool Maze::solve(Position p)
{
    if (maze[p.row][p.col] == 'F')
        return true;
    if (maze[p.row][p.col] == 'X' or maze[p.row][p.col] == 'b' or maze[p.row][p.col] == '.')
        return false;
    maze[p.row][p.col] = '.'; // breadcrumb
    if (solve(p.north()))
        return true;
    if (solve(p.east()))
        return true;
    ...
    if (solve(p.west()))
        return true;
    maze[p.row][p.col] = 'b'; // backtrack crumb
...
int Maze::solve_and_count(int row, int col)
{
    if (maze[row][col] == 'F')
        return 1;
    if (maze[row][col] == 'X' or maze[row][col] == 'b' or maze[row][col] == '.')
        return -1;
