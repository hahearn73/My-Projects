...
{
    for (size_t i = 0; i < artists.size(); i++) // make vertices
        insert_vertex(artists[i]);
    for (size_t i = 0; i < artists.size(); i++) {
        for (size_t j = 0; j < artists.size(); j++) { // find edges \/
            if (i != j and artists[i].get_collaboration(artists[j]) != "")
                insert_edge(artists[i], artists[j], // inserts edges
...
stack<Artist> CollabGraph::report_path(const Artist& source,
    ...
    stack<Artist> path; // for return
    if (source == dest) // check for duplicate
        return path;
    Vertex* sourceVertex = graph.at(source.get_name()); // vertex of source
    Vertex* destVertex = graph.at(dest.get_name()); // vertex of dest
    Vertex* curr = destVertex;
    while (curr != sourceVertex and curr != nullptr) {
        Artist ret(curr->artist);
        path.push(ret);
        curr = curr->predecessor;
    }
    path.push(sourceVertex->artist); // push source
    if (curr == nullptr) { // no path
        stack<Artist> path2;
        return path2; // return empty path
    }
    return path; // returns empty signifying no path found
}
